...
**测试文件**: `ephemeris/tests/test_jpl431_vs_swisseph_6000_years.rs`

**测试范围**:
- 时间跨度：3000 BC 至 3000 AD (6000 年)
- 采样方法：随机采样
- 测试天体：太阳、月球、行星
- 维度：视黄经 (Lon)、视黄纬 (Lat)、视赤经 (RA)、视赤纬 (Dec)、距离 (Dist)、经度角速度 (Speed)
...
通过引入 Vondrák 2011 岁差模型和 IAU 2000A 章动矩阵，视赤经纬的计算已达到极高精度：

| 参数 | 现代 (AD) 典型误差 | 古代 (BC) 典型误差 | 结论 |
|------|-------------------|-------------------|------|
| **视赤经 (RA)** | < 0.1" | < 1.0" | **对标精准** |
| **视赤纬 (Dec)** | < 0.05" | < 0.3" | **对标精准** |
| **角速度 (Speed)**| < 0.001"/day | < 1.2"/day | **符合动力学** |

**统计摘要**:
- **98%** 的测试点在视黄经/赤经维度误差小于 1 角秒。
- 远古时期（BC 3000）月球经度偏差约 70-90 角秒，确认为 DE431 与 Swiss Ephemeris 潮汐加速度理论参数差异，非实现错误。
...
#### c830335 - 实现视赤经纬计算并完成 6000 年全维度坐标对标

**类型**: feat  
**主要变更**:
1. **赤道坐标对标**: 实现 `transform_to_apparent_equatorial`，应用瞬时真赤道系统。
2. **速度计算暴露**: 在 `astronomy.rs` 中增加 `v_ra` 和 `v_dec` 字段，支持中心差分提取瞬时运动速率。
3. **API 完善**: `PlanetCoordinates` 现可输出完整的视位置及角速度信息。
4. **验证工具升级**: `test_jpl431_vs_swisseph_6000_years.rs` 现支持全维度参数自动化对比。

**技术细节**:
- 岁差：Vondrák 2011 (适用于 +/- 200,000 年，本项目取 6000 年验证)。
- 章动：IAU 2000A 完整矩阵。
- 速度：dt = 0.0001 day 中心差分。
