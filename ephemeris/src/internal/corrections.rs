...
pub fn precession_iau2006(t: f64) -> (f64, f64, f64) {
    ...
    let z_arcsec = -2.650545 + 2306.077181 * t + 1.0927348 * t2 + 0.01826837 * t3
        - 0.0000028596 * t.powi(4) - 0.0000002904 * t.powi(5);
    
    let theta_arcsec = 2004.191903 * t - 0.4294934 * t2 - 0.04182264 * t3
        - 0.0000070898 * t.powi(4) - 0.0000001274 * t.powi(5);
...
/// 应用岁差旋转到笛卡尔坐标
///
/// 使用 3-2-3 旋转序列：Rz(-z) * Ry(theta) * Rz(-zeta)
...
pub fn compute_lunar_apparent_position(
    ...
    // 回溯到光线发射时刻
    let current_t = t - light_time / 36525.0;
    let moon_retarded = m_coord(current_t, -1, -1, -1);
    ...
    let geo_xyz = (
        moon_xyz_au.0 - earth_xyz.0,
        moon_xyz_au.1 - earth_xyz.1,
        moon_xyz_au.2 - earth_xyz.2,
    );
